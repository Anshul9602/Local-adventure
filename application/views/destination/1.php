<style>
  .sppp1 .splide__arrows {
    display: none;
  }

  .new-font {
    font-size: 70px;
  }

  .des {
    border-bottom: 1px solid #D4030C;
  }

  h5 {
    padding-top: 10px;
    font-weight: 700;
    margin-bottom: 20px;
  }

  .desti .col-md-6 {
    padding: 0 4% 0
  }

  .tag {
    background-color: #D46503;
    padding: 1.5%;
    color: #fff;
    text-align: center;
    font-weight: 700;
    text-transform: uppercase;
  }

  .crd1 {
    margin-top: 5px;
    background-color: #D46503;
    padding: 5%;
    color: #fff;
    height: calc(33.33% - 5px);
    display: flex;
align-items: center;
  }

  .crd2 {
    margin-top: 5px;
    background-color: #FF8600;
    padding: 5%;
    color: #fff;
    height: calc(33.33% - 5px);
display: flex;
align-items: center;
  }
  .crd3 {
    margin-top: 5px;
    background-color: #D46503;
    padding: 5%;
    color: #fff;

    
  }
.crd1 .row{
  width: 100%;
}
.crd2 .row{
  width: 100%;
}
.crd3 .row{
  width: 100%;
}
.crd4 .row{
  width: 100%;
}

  .crd4 {
    margin-top: 5px;
    background-color: #FF8600;
    padding: 5%;
    color: #fff;

  }
  @media (max-width: 768px) {
    .new-font {
    font-size: 32px;
}
.tag h1{
  font-size: 18px;
  margin: 0;
}
    .crd1 {
   
    height: auto;
  
  }

  .crd2 {
   height: auto;
  }
.meel h1{
font-size: 20px;
}
.meel li{
  font-size: 12px;
}
.meel p{
  font-size: 12px;
}
  }
</style>
<div class="imm desk" style="position: relative;">
  <img src="assets/img/des1.png" alt="" width="100%">
  <div class="connn" style="    position: absolute;
    top: 5%;    height: 95%;
    width: 100%;">
    <div class="container" style="position: relative;    height: 100%;">
      <img src="assets/img/des3.png" alt="" width="100%" style="opacity: 0.4;height:100%;">
      <div class="connn" style="padding:5%;position: absolute;top: 0;height: fit-content;width: 100%;">
        <div class="row " style="color:#fff;">
          <div class="col-md-12">
            <h4 class="new-font text-center">Pathways to the Past</h4>
          </div>
          <div class="row mt-md-5 desti">
            <div class="col-md-6" style="border-right:1px solid #FFFFFF;">
              <h5>Day 1: Arrival in Delhi</h5>
              <p>Arrive in Delhi and attend a welcome meeting at 6 PM. Optionally, explore the city with Urban Adventures or join the group for dinner.
              </p>
              <h5>Day 2: Explore Old Delhi</h5>
              <p>Explore Old Delhi on a walking tour, visit Jama Masjid, ride a cycle rickshaw, and experience Sheeshganj Gurudwara’s communal kitchen. Free time in the afternoon for food walks or sightseeing.
              </p>
              <h5>Day 3: Agra – The City of Mughal Splendor</h5>
              <p>Take an express train to Agra. Visit the Taj Mahal, explore Agra Fort, and enjoy a motorized rickshaw ride. Optional visit to Akbar’s Mausoleum or Sheroes Hangout.
              </p>
              <h5>Day 4: Madhogarh – A Royal Retreat</h5>
              <p>Travel to rural Madhogarh, explore the village, and stay at a royal heritage property with an included dinner.
              </p>
              <h5>Day 5: Jaipur – The Pink City</h5>
              <p>Head to Jaipur, explore the bustling bazaars, and visit Amber Fort with its stunning Sheesh Mahal. Enjoy free time in the Pink City.

              </p>
              <h5>Day 6: Jaipur – A Day to Explore</h5>
              <p>Spend the day as you choose: take a hot air balloon ride, visit Hawa Mahal, Jantar Mantar, or catch a Bollywood film at Raj Mandir Cinema.


              </p>
              <h5>Day 7: Ranthambhore National Park Into to the Wild
              </h5>
              <p>Travel to Ranthambhore for a 4WD safari to spot tigers and other wildlife. Visit the park's ancient fort.


              </p>
            </div>
            <div class="col-md-6 mt-md-0 mt-3">
              <h5>Day 8: Bundi – Step Into Tradition</h5>
              <p>Discover Bundi’s markets, step wells, and Bundi Palace on a walking tour. Stay overnight in a heritage property.
              </p>
              <h5>Day 9: Paangarh Lake – A Serene Escape
              </h5>
              <p>Take a local train to Paangarh Lake and stay in a tented camp by the lakeshore, enjoying serene views and sunset chai.
              </p>
              <h5>Day 10: Bijaipur – Countryside Bliss</h5>
              <p>Explore the countryside by open jeep, meet villagers, and enjoy a traditional meal at a heritage property.
              </p>
              <h5>Day 11: Udaipur – The City of Lakest</h5>
              <p>Travel to Udaipur, the city of lakes, and explore its bustling bazaars, artistic streets, and cultural hubs.
              </p>
              <h5>Day 12: Udaipur – A Day to Indulge</h5>
              <p>Visit the City Palace, take an optional boat ride on Lake Pichola, or attend a Rajasthani folk dance performance.

              </p>
              <h5>Day 13: Pushkar – A Spiritual Retreat</h5>
              <p>Reach Pushkar, visit the sacred ghats and Brahma Temple, and share dinner with a local family.
              </p>
              <h5>Day 14: Pushkar to Delhi Until Next Time
              </h5>
              <p>Hike to Savitri Temple for sunrise views, then board a train to Delhi for your final night.
              </p>
            </div>
            <div class="col-md-12 row text-center mt-4 justify-content-around">
              <div class="col-md-6">
                <h5>Day 15: Departure from Delhi</h5>
                <p>Your adventure concludes today. Check out at your convenience and depart with memories to last a lifetime.
                </p>
              </div>

            </div>
          </div>

        </div>
      </div>
    </div>
  </div>
  <div class="conn" style="    position: absolute;
    bottom: 0%; width:100%;  ">
    <img src="assets/img/des2.png" alt="" width="100%">
  </div>
</div>


<div class="imm mob" style="position: relative;    background-image: url('assets/img/des1.png');background-size: cover;">

  <div class="connn pt-5 mt-5">
    <div class="container pt-5 " style="position: relative;    height: 100%;background-image: url('assets/img/des3.png');background-size: cover;">

      <div class="connn">
        <div class="row " style="color:#fff;">
          <div class="col-md-12">
            <h6 class="new-font text-start">Pathways to the Past</h6>
          </div>
          <div class="row mt-md-5 desti">
            <div class="col-md-6" >
              <h5>Day 1: Arrival in Delhi</h5>
              <p>Arrive in Delhi and attend a welcome meeting at 6 PM. Optionally, explore the city with Urban Adventures or join the group for dinner.
              </p>
              <h5>Day 2: Explore Old Delhi</h5>
              <p>Explore Old Delhi on a walking tour, visit Jama Masjid, ride a cycle rickshaw, and experience Sheeshganj Gurudwara’s communal kitchen. Free time in the afternoon for food walks or sightseeing.
              </p>
              <h5>Day 3: Agra – The City of Mughal Splendor</h5>
              <p>Take an express train to Agra. Visit the Taj Mahal, explore Agra Fort, and enjoy a motorized rickshaw ride. Optional visit to Akbar’s Mausoleum or Sheroes Hangout.
              </p>
              <h5>Day 4: Madhogarh – A Royal Retreat</h5>
              <p>Travel to rural Madhogarh, explore the village, and stay at a royal heritage property with an included dinner.
              </p>
              <h5>Day 5: Jaipur – The Pink City</h5>
              <p>Head to Jaipur, explore the bustling bazaars, and visit Amber Fort with its stunning Sheesh Mahal. Enjoy free time in the Pink City.

              </p>
              <h5>Day 6: Jaipur – A Day to Explore</h5>
              <p>Spend the day as you choose: take a hot air balloon ride, visit Hawa Mahal, Jantar Mantar, or catch a Bollywood film at Raj Mandir Cinema.


              </p>
              <h5>Day 7: Ranthambhore National Park Into to the Wild
              </h5>
              <p>Travel to Ranthambhore for a 4WD safari to spot tigers and other wildlife. Visit the park's ancient fort.


              </p>
            </div>
            <div class="col-md-6 mt-md-0 mt-3">
              <h5>Day 8: Bundi – Step Into Tradition</h5>
              <p>Discover Bundi’s markets, step wells, and Bundi Palace on a walking tour. Stay overnight in a heritage property.
              </p>
              <h5>Day 9: Paangarh Lake – A Serene Escape
              </h5>
              <p>Take a local train to Paangarh Lake and stay in a tented camp by the lakeshore, enjoying serene views and sunset chai.
              </p>
              <h5>Day 10: Bijaipur – Countryside Bliss</h5>
              <p>Explore the countryside by open jeep, meet villagers, and enjoy a traditional meal at a heritage property.
              </p>
              <h5>Day 11: Udaipur – The City of Lakest</h5>
              <p>Travel to Udaipur, the city of lakes, and explore its bustling bazaars, artistic streets, and cultural hubs.
              </p>
              <h5>Day 12: Udaipur – A Day to Indulge</h5>
              <p>Visit the City Palace, take an optional boat ride on Lake Pichola, or attend a Rajasthani folk dance performance.

              </p>
              <h5>Day 13: Pushkar – A Spiritual Retreat</h5>
              <p>Reach Pushkar, visit the sacred ghats and Brahma Temple, and share dinner with a local family.
              </p>
              <h5>Day 14: Pushkar to Delhi Until Next Time
              </h5>
              <p>Hike to Savitri Temple for sunrise views, then board a train to Delhi for your final night.
              </p>
            </div>
            <div class="col-md-12 row text-center mt-4 justify-content-around">
              <div class="col-md-6">
                <h5>Day 15: Departure from Delhi</h5>
                <p>Your adventure concludes today. Check out at your convenience and depart with memories to last a lifetime.
                </p>
              </div>

            </div>
          </div>

        </div>
      </div>

    </div>
    <div class="conn" style="width: 100%;margin-top: -20%;z-index: 99;position: relative;bottom: 0%; width:100%;  ">
      <img src="assets/img/des2.png" alt="" width="100%">
    </div>
  </div>

</div>



<div class="tag">
  <h1>Golden Triangle Of India</h1>
</div>

<section class="meel">
  <div class="container">
    <div class="row">
      <div class="col-md-6 ">
        <div class="crd1">
          <div class="row" >
            <div class="col-md-2 col-4">
              <img src="assets/img/desti/meal.png" alt="">
            </div>
            <div class="col-md-8 col-8">
              <h1>Meal</h1>
              <p>Breakfast</p>
            </div>
          </div>
        </div>
        <div class="crd2">
          <div class="row">
            <div class="col-md-2 col-4">
              <img src="assets/img/desti/hotel.png" alt="">
            </div>
            <div class="col-md-8 col-8">
              <h1>Accommodation</h1>
              <p>Hotel (6 nights),</p>
              <p>Heritage Property (1 night)</p>
            </div>
          </div>
        </div>
        <div class="crd1">
          <div class="row">
            <div class="col-md-2 col-4">
              <img src="assets/img/desti/bus.png" alt="">
            </div>
            <div class="col-md-8 col-8">
              <h1>Transport</h1>
              <p>Private vehicle, Shuttle Bus,
                Cycle rickshaw, Auto rickshaw, Metro</p>
            </div>
          </div>
        </div>
      </div>
      <div class="col-md-6 ">
        <div class="crd4">
          <div class="row">
            <div class="col-md-2 col-4">
              <img src="assets/img/desti/meal.png" alt="">
            </div>
            <div class="col-md-10 col-8">
              <h1>Included activities</h1>
              <ul>
                <li>Delhi - Visit to the Jama Masjid Mosque</li>
                <li>Delhi - Sheeshganj Gurudwara (Sikh Temple)</li>
                <li>Delhi - Leader-led Old Delhi walking tour</li>
                <li>Jaipur - Leader-led walking & market tour</li>
                <li>Jaipur - Amber Fort</li>
                <li>Suroth Mehal - Village Walk</li>
                <li>Fatehpur Sikri - Guided tour</li>
                <li>Agra - Taj Mahal</li>
                <li>Agra - Agra Fort</li>
                <li>Agra - Sheroes Cafe Visit</li>
              </ul>
              
            </div>
          </div>
        </div>
        <div class="crd3">
          <div class="row">
            <div class="col-md-2 col-4">
              <img src="assets/img/desti/ex.png" alt="">
            </div>
            <div class="col-md-8 col-8">
              <h1>Exclusions</h1>
              <ul>
                <li>Tips to the Driver and Guide.</li>
                <li>Any Expenses of personal mature</li>
                <li>Visa</li>
                <li>International Flight</li>
                <li>Any thing not mentioned in inclusions</li>
              </ul>
              
            </div>
          </div>
        </div>
        
      </div>
     
    </div>
  </div>
</section>